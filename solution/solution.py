...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an expert judge evaluating an AI assistant's answer.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            "Rubric criteria (score each from 0.0 to 1.0):\n"
            f"{criteria}\n\n"
            "Respond with a JSON object mapping each criterion name to a "
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            entry_averages.append(sum(values) / len(values) if values else 0.0)

        positional_bias = len(entry_averages) > 1 and all(
            entry_averages[0] > avg for avg in entry_averages[1:]
        )
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def avg_retrieval(attr: str) -> float | None:
            values = [
        ...
        return {
            ...
            "avg_context_recall": avg_retrieval("context_recall"),
            "avg_context_precision": avg_retrieval("context_precision"),
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            fix = suggestions[index] if index < len(suggestions) else "N/A"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter claims not "
                "supported by retrieved context"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and intent detection so answers "
                "directly address the question asked"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or top-k in the RAG pipeline to reduce "
                "context fragmentation and cover more evidence"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review routing/intent classification — answers are "
                "addressing a different topic than what was asked"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Relax overly strict guardrails that cause valid questions "
                "to be refused"
            )
        ...
        suggestions.append(
            "Expand golden dataset coverage for the most frequent failure "
            "category to track regressions"
        )
        suggestions.append(
            "Re-run the benchmark after each fix and compare against "
            "baseline with run_regression()"
        )
        return suggestions
